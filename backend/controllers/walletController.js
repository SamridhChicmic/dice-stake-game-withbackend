...
import { demoStore, getDemoOrder, isDemoMode } from "../config/demo.js";
export const OnRampTransactionsUpdated = async (req, res) => {
    ...
    if (isDemoMode) {
        ...
        demoStore.onRampTransactions.unshift(transaction);
        demoStore.balance.amount += amount;
...
try {
   ...
   // console.log(amount)
    const options = {
      ...
      if (isDemoMode) {
        const order = getDemoOrder(options);
        return res.status(200).json({
            success: true,
            order,
        });
...
export const getOnRampTransactions = async (req, res) => {
    ...
    if (isDemoMode) {
        return res.status(200).json(demoStore.onRampTransactions);
    }
...
export const get5Transactions = async (req, res) => {
    ...
    if (isDemoMode) {
        return res.status(200).json(demoStore.onRampTransactions.slice(0,5));
    }
    const txns = await prisma.onRampTransaction.findMany({
