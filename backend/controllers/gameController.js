...
export const diceLogic = async (req, res) => {
    ...
    if (isDemoMode) {
        ...
        demoStore.balance.amount = parseInt(newBalance);
        demoStore.gameTransactions.unshift({
        ...
        return res.status(200).json({
            ...
            roll,
            payout,
            newBalance: demoStore.balance.amount,
        });
    }
...
export const getGamesTxn=async(req,res)=>{
    try {
      if (isDemoMode) {
        return res.status(200).json(demoStore.gameTransactions.slice(0,5));
      }
